...
function SmelterDel()
    Citizen.CreateThread(function()
        local str = Config.Smelter.PromptCancelName
...
function SmelterSet()
    Citizen.CreateThread(function()
        local str = Config.Smelter.PromptPlaceName
...
function SmelterRotateLeft()
    Citizen.CreateThread(function()
        local str = Config.Smelter.PromptRotateLeft
...
function SmelterRotateRight()
    Citizen.CreateThread(function()
        local str = Config.Smelter.PromptRotateRight
...
function SmelterModelRequest(model)
    Citizen.CreateThread(function()
        RequestModel(model)
    end)
end

function SmelterPropPlacer(proptype, prop)
    ...
    local x, y, z = table.unpack(pos - forward * -Config.Smelter.ForwardDistance)
    local ox = x - _x
    local oy = y - _y
    local oz = z - _z
    local heading = 0.0
    ...
    while true do
        Wait(5)
        local PropPlacerGroupName = CreateVarString(10, 'LITERAL_STRING', Config.Smelter.PromptGroupName)
        ...
        if PromptHasHoldModeCompleted(SetPrompt) then
            ...
            TriggerEvent('rsg-goldclaim:smelter:client:placeNewProp', proptype, PropHash, pPos, heading)
...
RegisterNetEvent('rsg-goldclaim:smelter:client:createprop', function(proptype, prop)
    SmelterPropPlacer(proptype, prop)
end)
